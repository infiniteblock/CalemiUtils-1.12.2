...
public class BlockNetworkCable extends BlockNetworkCableOpaque {
	...
	private static final AxisAlignedBB CORE_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, pixel * 5, pixel * 11,
			pixel * 11, pixel * 11);

	private static final AxisAlignedBB DOWN_AABB = new AxisAlignedBB(pixel * 5, pixel * 0, pixel * 5, pixel * 11,
			pixel * 11, pixel * 11);
	private static final AxisAlignedBB UP_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, pixel * 5, pixel * 11,
			pixel * 16, pixel * 11);
	private static final AxisAlignedBB NORTH_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, 0, pixel * 11, pixel * 11,
			pixel * 5);
	private static final AxisAlignedBB EAST_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, pixel * 5, pixel * 16,
			pixel * 11, pixel * 11);
	private static final AxisAlignedBB SOUTH_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, pixel * 5, pixel * 11,
			pixel * 11, pixel * 16);
	private static final AxisAlignedBB WEST_AABB = new AxisAlignedBB(pixel * 0, pixel * 5, pixel * 5, pixel * 11,
			pixel * 11, pixel * 11);
	private static final AxisAlignedBB DOWNUP_AABB = new AxisAlignedBB(pixel * 5, pixel * 0, pixel * 5, pixel * 11,
			pixel * 16, pixel * 11);
	private static final AxisAlignedBB NORTHSOUTH_AABB = new AxisAlignedBB(pixel * 5, pixel * 5, pixel * 0, pixel * 11,
			pixel * 11, pixel * 16);
	private static final AxisAlignedBB EASTWEST_AABB = new AxisAlignedBB(pixel * 0, pixel * 5, pixel * 5, pixel * 16,
			pixel * 11, pixel * 11);

	public BlockNetworkCable() {

		super("network_cable", true);
		setDefaultState(this.blockState.getBaseState().withProperty(DOWN, Boolean.FALSE).withProperty(UP, Boolean.FALSE)
				.withProperty(NORTH, Boolean.FALSE).withProperty(EAST, Boolean.FALSE).withProperty(SOUTH, Boolean.FALSE)
				.withProperty(WEST, Boolean.FALSE).withProperty(DOWNUP, Boolean.FALSE)
	...
	private static AxisAlignedBB getBoundingBoxIdx(IBlockState state) {
		...
		if (state.getValue(DOWNUP))
			i = DOWNUP_AABB;
		if (state.getValue(NORTHSOUTH))
			i = NORTHSOUTH_AABB;
		if (state.getValue(EASTWEST))
			i = EASTWEST_AABB;
	...
	public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox,
		...
		if (state.getValue(DOWNUP))
			addCollisionBoxToList(pos, entityBox, collidingBoxes, DOWNUP_AABB);
		if (state.getValue(NORTHSOUTH))
			addCollisionBoxToList(pos, entityBox, collidingBoxes, NORTHSOUTH_AABB);
		if (state.getValue(EASTWEST))
			addCollisionBoxToList(pos, entityBox, collidingBoxes, EASTWEST_AABB);
	}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		return state.withProperty(DOWN, down).withProperty(UP, up).withProperty(NORTH, north).withProperty(EAST, east)
				.withProperty(SOUTH, south).withProperty(WEST, west).withProperty(DOWNUP, downup)
				.withProperty(NORTHSOUTH, northsouth).withProperty(EASTWEST, eastwest);
	}
