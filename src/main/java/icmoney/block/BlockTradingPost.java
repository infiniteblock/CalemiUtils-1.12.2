...
public class BlockTradingPost extends BlockInventoryContainerBase implements IExtraInformation {
	...
	private static final AxisAlignedBB AABB = new AxisAlignedBB(pixel * 0, pixel * 0, pixel * 0, pixel * 16, pixel * 3,
			pixel * 16);
		...
		if (placer instanceof EntityPlayer) {
			...
			if (player.capabilities.isCreativeMode && !player.isSneaking()) {
				...
				if (te instanceof TileEntityTradingPost) {
					...
					if (!worldIn.isRemote)
						tePost.getUnitName(player).printMessage(TextFormatting.GREEN,
								"Admin Mode is enabled for this block. Sneak place this block to disable it.");
				}
			}
		}
	}
		...
		if (te instanceof TileEntityTradingPost) {
			...
			if (!player.isSneaking() && heldStack.getItem() == InitItems.SECURITY_WRENCH) {
				...
				if (SecurityHelper.openSecuredBlock(location, player, true)) {
					FMLNetworkHandler.openGui(player, ICMoney.instance, 0, world, pos.getX(), pos.getY(), pos.getZ());
				}
			}
				...
				if (tePost.hasValidTradeOffer) {
					...
					if (tePost.adminMode) {
						message.printMessage(TextFormatting.GREEN, (tePost.buyMode ? "Buying " : "Selling ")
								+ StringHelper.printCommas(tePost.amountForSale) + " "
								+ tePost.getStackForSale().getDisplayName() + " for "
								+ (tePost.salePrice > 0 ? (StringHelper.printCurrency(tePost.salePrice)) : "free"));
					}

					else
						message.printMessage(TextFormatting.GREEN, tePost.getSecurityProfile().getOwnerName() + " is "
								+ (tePost.buyMode ? "buying " : "selling ")
								+ StringHelper.printCommas(tePost.amountForSale) + " "
								+ tePost.getStackForSale().getDisplayName() + " for "
								+ (tePost.salePrice > 0 ? (StringHelper.printCurrency(tePost.salePrice)) : "free"));
	...
	private void handlePurchase(UnitChatMessage message, ItemStack walletStack, World world, EntityPlayer player,
				...
				if (tePost.getStoredCurrencyInBank() + tePost.salePrice < ICMConfig.misc.postCurrencyCapacity) {
					...
					if (tePost.adminMode) {
						...
						if (count >= tePost.amountForSale) {
							...
							InventoryHelper.consumeItem(0, tePost, tePost.amountForSale, true, true,
									tePost.getStackForSale());
	...
	private void handleSell(UnitChatMessage message, ItemStack walletStack, World world, EntityPlayer player,
			TileEntityTradingPost tePost) {

		if (InventoryHelper.countItems(player.inventory, true, true,
				tePost.getStackForSale()) >= tePost.amountForSale) {
			...
			if (InventoryHelper.canInsertItem(is, tePost) || tePost.adminMode) {
				...
				if (CurrencyHelper.canFitAddedCurrencyToWallet(walletStack, tePost.salePrice)) {
					...
					if (tePost.getStoredCurrencyInBank() >= tePost.salePrice
							|| tePost.getStoredCurrencyInBank() >= tePost.salePrice || tePost.adminMode) {

						NBTTagCompound nbt = ItemHelper.getNBT(walletStack);

						InventoryHelper.consumeItem(player.inventory, tePost.amountForSale, true,
								tePost.getStackForSale());
			...
			message.printMessage(TextFormatting.RED,
					"You do not have the required item(s) the Trading Post is looking for!");
	}
