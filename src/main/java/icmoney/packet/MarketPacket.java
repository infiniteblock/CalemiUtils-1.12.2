		...
		public IMessage onMessage(MarketPacket message, MessageContext ctx) {
			...
			if (data[0].equalsIgnoreCase("syncamount")) {
				...
				if (data.length > 6) {
					...
					ItemHelper.getNBT(stack).setInteger("balance",
							ItemHelper.getNBT(stack).getInteger("balance") - (value * purchaseAmount));
				}

				ItemHelper.spawnItem(world, player,
						new ItemStack(Item.getByNameOrId(itemObj), amount * purchaseAmount, meta));
			}
				...
				if (data.length > 6) {
					...
					ItemHelper.getNBT(walletStack).setInteger("balance",
							ItemHelper.getNBT(walletStack).getInteger("balance") + (value * purchaseAmount));
				}
