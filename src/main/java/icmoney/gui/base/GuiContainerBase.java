...
public abstract class GuiContainerBase extends GuiContainer {
	...
	private void addProgressBarText(int mouseX, int mouseY, int progress, int maxProgress) {
		...
		GuiRect rect = new GuiRect(getScreenX() - 13, getScreenY() + leftTabOffset, 13, 35);
		GuiHelper.drawHoveringTextBox(mouseX, mouseY, 170, rect,
				"Progress: " + MathHelper.scaleInt(progress, maxProgress, 100) + "%");
	}
	...
	protected void addRightInfoText(String text, int sizeAdd, int sizeY) {
		...
		GuiHelper.bindGuiTextures();
		GuiHelper.drawCappedRect(getScreenX() + getGuiSizeX(), getScreenY() + rightTabOffset, 0, 116, 2, width, sizeY,
				256, 22);
		...
		if (!text.isEmpty()) {
			...
			mc.fontRenderer.drawString(text, getScreenX() + getGuiSizeX() + 4,
					getScreenY() + (sizeY / 2) - 3 + rightTabOffset, TEXT_COLOR);
	...
	private void addLeftInfoText(String text, int sizeY) {
		...
		if (!text.isEmpty()) {
			...
			GL11.glColor3f(0.35F, 0.35F, 0.35F);
			GL11.glTranslatef(0, 0, 5);
			mc.fontRenderer.drawString(text, getScreenX() - width + 4, getScreenY() + (sizeY / 2) - 3 + leftTabOffset,
					TEXT_COLOR);
			GL11.glColor3f(1, 1, 1);
			GlStateManager.popMatrix();
		}
	...
	private void addGraphicsAfterRendering(int mouseX, int mouseY) {
		...
		if (te != null) {
			...
			if (te instanceof ISecurity) {
				...
				GuiHelper.drawCappedRect(getScreenX() + (getGuiSizeX() / 2) - (width / 2), getScreenY() + getGuiSizeY(),
						0, 116, 0, width, 13, 256, 22);
				GuiHelper.drawCenteredString(name, getScreenX() + (getGuiSizeX() / 2), getScreenY() + getGuiSizeY() + 3,
						TEXT_COLOR);
				GL11.glColor3f(1, 1, 1);
			}
			...
			if (te instanceof ICurrencyNetworkBank) {
				GL11.glColor3f(1, 1, 1);
				addCurrencyInfo(mouseX, mouseY, ((ICurrencyNetworkBank) te).getStoredCurrency(),
						((ICurrencyNetworkBank) te).getMaxCurrency());
			}

			if (te instanceof IProgress) {
				GL11.glColor3f(1, 1, 1);
				addProgressBarText(mouseX, mouseY, ((IProgress) te).getCurrentProgress(),
						((IProgress) te).getMaxProgress());
			}
		}
	}
