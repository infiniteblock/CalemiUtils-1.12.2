...
public class GuiWallet extends GuiContainerBase {
	...
	public void initGui() {
		...
		toggleSuckButton = new GuiButtonRect(4, getScreenX() + (getGuiSizeX() / 2) - (54 / 2) - 54,
				getScreenY() + 18 + (3 * 18), 54, "", buttonList);
		setActiveButton = new GuiButtonRect(5, getScreenX() + (getGuiSizeX() / 2) - (48 / 2),
				getScreenY() + 18 + (3 * 18), 48, "Activate", buttonList);
	}
	...
	protected void actionPerformed(GuiButton button) {
		...
		if (!walletStack.isEmpty()) {
			...
			if (button.id < 4 && ItemWallet.getBalance(walletStack) >= price) {
				...
				ICMoney.network.sendToServer(new WalletPacket("withdraw%" + button.id + "%" + multiplier));
	...
	public void drawGuiBackground(int mouseX, int mouseY) {
		...
		GuiHelper.drawItemStack(itemRender, new ItemStack(InitItems.COIN_NICKEL), getScreenX() + 127,
				getScreenY() + 33);
		GuiHelper.drawItemStack(itemRender, new ItemStack(InitItems.COIN_QUARTER), getScreenX() + 127,
				getScreenY() + 51);
		GuiHelper.drawItemStack(itemRender, new ItemStack(InitItems.COIN_DOLLAR), getScreenX() + 127,
				getScreenY() + 69);
	...
	public void drawGuiForeground(int mouseX, int mouseY) {
		...
		if (!ItemWallet.isActive(getCurrentWalletStack())) {
			...
			leftTabOffset += 17;
			addInfoIcon(1);
			addInfoIconText(mouseX, mouseY, "Inactive!", "Press the activate button.",
					"There can only be one active Wallet.");
		}
	}
}
