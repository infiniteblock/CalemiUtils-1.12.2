...
public class TileEntityTradingPost extends TileEntityInventoryBase
		implements ITileEntityGuiHandler, ICurrencyNetworkUnit, ISecurity {
			...
			for (int i = 0; i < getSizeInventory(); i++) {
				...
				if (getStackInSlot(i) != null && getStackInSlot(i).isItemEqual(getStackForSale())) {
					...
					if (getStackForSale().hasTagCompound()) {
						...
						if (getStackInSlot(i).hasTagCompound()
								&& getStackInSlot(i).getTagCompound().equals(getStackForSale().getTagCompound())) {
	...
	public ITextComponent getDisplayName() {
		...
		if (hasValidTradeOffer) {
			return new TextComponentString((buyMode ? "Buying " : "Selling ") + amountForSale + "x "
					+ getStackForSale().getDisplayName() + " for " + StringHelper.printCurrency(salePrice));
		}
